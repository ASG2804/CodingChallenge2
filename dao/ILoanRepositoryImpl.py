...
from .Exception import (
    ApplyLoneException,
    calculateEMIException,
    InvalidLoanException,
    RetriveloanException,
    calculateInterestException,
)
...
class ILoanRepositoryImpl(ILoanRepository):
    ...
    def calculateInterest(self, LoanID):
        try:
            ...
            self.connection.rollback()
            raise calculateInterestException(
                "Couldn't calculate interest. Error: {}".format(str(e))
            )

    def loanStatus(self, loanId):
        try:
            self.cursor.execute(
                "Select credit_score from Loan l inner join  Customer c on c.customerID=l.customerID where loanID=?;",
                (loanId),
            )
            result = self.cursor.fetchone()
            if result:
            ...
            raise InvalidLoanException(
                "Couldn't find customer/loan. Error: {}".format(str(e))
            )
    ...
    def calculateEMI(self, loneId):
        try:
            ...
            if result:
                ...
                emi = (principalAmount * R * (1 + R) ** loanTerm) / (
                    (1 + R) ** loanTerm - 1
                )
    ...
    def loanRepayment(self, loneId, amount):
        try:
            ...
            if emi:
                if emi > amount:
                    raise InvalidLoanException(
                        f"Payment rejected amount {amount} is letter than emi {emi}"
                    )
                else:
                    while amount > 0:
                        amount = amount / emi
                ...
                print(
                    f"The total number of emi payed by amount {amount} is {loanRepaymentcount}"
                )
            else:
